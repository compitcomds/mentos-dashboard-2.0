...
const SORT_FIELD_OPTIONS: { label: string; value: SortField }[] = [
  { label: "Created At", value: "createdAt" }, { label: "Updated At", value: "updatedAt" },
  { label: "Published At", value: "publishedAt" }, { label: "Title", value: "title" },
];
...
export default function BlogPage() {
   ...
   const viewMode = (searchParams.get('view') as ViewMode | null) || getStoredPreference('blogViewMode', 'table');
   const currentPage = parseInt(searchParams.get('page') || '1', 10);
   const pageSize = parseInt(searchParams.get('limit') || String(getStoredPreference('blogPageSize', viewMode === 'table' ? DEFAULT_PAGE_SIZE_BLOG_TABLE : DEFAULT_PAGE_SIZE_BLOG_CARD)), 10);
   const sortField = (searchParams.get('sortBy') as SortField | null) || getStoredPreference('blogSortField', DEFAULT_SORT_FIELD);
   const sortOrder = (searchParams.get('order') as SortOrder | null) || getStoredPreference('blogSortOrder', DEFAULT_SORT_ORDER);
   const searchTerm = searchParams.get('search') || '';
   const selectedCategoryId = searchParams.get('category') || null;

   const [localSearchTerm, setLocalSearchTerm] = React.useState(searchTerm);
   React.useEffect(() => { setLocalSearchTerm(searchTerm); }, [searchTerm]);
   ...
   const updateUrl = React.useCallback((newParams: Record<string, string | number | null>) => {
    ...
    if (newParams.page === 1 || newParams.page === '1') current.delete('page'); // Remove page if it's 1
    router.push(`${pathname}?${current.toString()}`, { scroll: false });
  }, [router, pathname, searchParams]);

  const handleViewModeChange = (newMode: ViewMode) => { updateUrl({ view: newMode }); setStoredPreference('blogViewMode', newMode); };
  const handlePageChange = (newPage: number) => updateUrl({ page: newPage });
  const handlePageSizeChange = (value: string) => { updateUrl({ limit: value, page: null }); setStoredPreference('blogPageSize', Number(value)); };
  const handleSortFieldChange = (value: SortField) => { updateUrl({ sortBy: value, page: null }); setStoredPreference('blogSortField', value); };
  const handleSortOrderChange = (value: SortOrder) => { updateUrl({ order: value, page: null }); setStoredPreference('blogSortOrder', value); };
  const handleCategoryFilterChange = (value: string | null) => { updateUrl({ category: value, page: null }); };
  const applySearchTerm = () => updateUrl({ search: localSearchTerm.trim() || null, page: null });
    ...
    <TooltipProvider>
        ...
        <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-2">
                ...
                <Link href={`/dashboard/blog/new?returnUrl=${encodeURIComponent(currentFullUrl)}`}>
                        ...
                        <AccordionContent className="pt-3 space-y-3">
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-2 items-end">
                                <div className="relative md:col-span-1">
                                    ...
                                    <Input id="search-blogs" type="search" placeholder="Search..." value={localSearchTerm} onChange={(e) => setLocalSearchTerm(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && applySearchTerm()} className="pl-8 h-8 text-xs" disabled={isLoadingBlogs || isFetching} />
                                    {localSearchTerm && (<Button variant="ghost" size="icon" className="absolute right-1 top-1/2 h-6 w-6 -translate-y-1/2 mt-2.5" onClick={() => { setLocalSearchTerm(''); updateUrl({ search: null, page: null }); }}><X className="h-3.5 w-3.5" /><span className="sr-only">Clear search</span></Button>)}
                                </div>
                                <div>
                                    ...
                                    <Select 
                                        ...
                                        disabled={isLoadingCategories || !categories || categories.length === 0 || isLoadingBlogs || isFetching}
                                    >
                                ...
                                <Button onClick={applySearchTerm} size="sm" className="w-full md:w-auto h-8 text-xs px-3 py-1" disabled={isLoadingBlogs || isFetching}><Search className="h-3.5 w-3.5 mr-1.5" /> Apply Search</Button>
                            </div>
                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 items-end pt-3 border-t mt-3">
                                <div>
                                    <Label className="text-xs text-muted-foreground mb-1 block">Sort By</Label>
                                    <Select value={sortField} onValueChange={(value) => handleSortFieldChange(value as SortField)} disabled={isLoadingBlogs || isFetching}>
                                ...
                                <div>
                                    ...
                                    <Select value={sortOrder} onValueChange={(value) => handleSortOrderChange(value as SortOrder)} disabled={isLoadingBlogs || isFetching}>
                                ...
                                <div>
                                    ...
                                    <Select value={String(pageSize)} onValueChange={handlePageSizeChange} disabled={isLoadingBlogs || isFetching}>
        ...
        {isError && !isFetching && (
         ...
         {!isLoading && !isError && userTenentId && filteredBlogPosts.length === 0 && (
            ...
            {blogData?.data && blogData.data.length > 0 ? 'No blog posts match your current filters.' : `No blog posts found for your tenent_id (${userTenentId}).`}
            </div>
         )}
                ...
                <CardContent>
                    ...
                    <TableBody>
                        {filteredBlogPosts.map((post) => {
                            ...
                            const editLink = `/dashboard/blog/${post.documentId || post.id}?returnUrl=${encodeURIComponent(currentFullUrl)}`;
