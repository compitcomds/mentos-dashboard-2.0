...
import {
  ...
  AlertTitle as AlertTitleComponentUI,
...
const getFileTypeRenderIcon = (
  ...
  if (mime.startsWith("image/"))
    return <FileTypeIcon className={cn(iconClasses, "text-blue-500")} />;
  if (mime.startsWith("video/"))
    return <Video className={cn(iconClasses, "text-purple-500")} />;
...
export default function MediaSelectorDialog({
  ...
  const allAvailableTagsForDialog = React.useMemo(() => {
    return Array.from(
      new Set([...PREDEFINED_TAGS_FOR_WEB_MEDIA, ...userDefinedTags])
    ).sort();
  }, [userDefinedTags]);

  const {
    data: allMediaDataResponse,
  ...
  const [selectedMediaInDialog, setSelectedMediaInDialog] =
    React.useState<CombinedMediaData | null>(null);
  ...
  const handleConfirmSelection = () => {
    ...
    if (
      returnType === "id" &&
      (selectedMediaInDialog.fileId === null ||
        selectedMediaInDialog.fileId === undefined)
    ) {
      console.error(
        "MediaSelectorDialog: Attempted to select media with null fileId when returnType is 'id'."
      );
      return;
    }
    if (returnType === "url" && !selectedMediaInDialog.fileUrl) {
      console.error(
        "MediaSelectorDialog: Attempted to select media with null fileUrl when returnType is 'url'."
      );
      return;
    }
  ...
  const handleOpenChange = (open: boolean) => {
    onOpenChange(open);
    if (!open) setSelectedMediaInDialog(null);
  };
  ...
  const canConfirmSelection =
    selectedMediaInDialog !== null &&
    (returnType === "id"
      ? selectedMediaInDialog.fileId !== null &&
        selectedMediaInDialog.fileId !== undefined
      : !!selectedMediaInDialog.fileUrl);
      ...
      >
        ...
        <ScrollArea className="flex-1 px-3 py-8 overflow-scroll bg-slate-200 ">
          <div className="min-h-[200px]">
            {" "}
            {/* Ensure some min height for loading/empty states */}
            ...
            {!isLoading &&
              !isError &&
              (!filteredMediaData || filteredMediaData.length === 0) && (
                <p className="text-center text-muted-foreground py-10 text-sm">
                  {userKey
                    ? `No media files found${
                        nameFilter || selectedFilterTags.length > 0
                          ? " matching filters"
                          : expectedMediaTypes.length > 0
                          ? ` for type: ${expectedMediaTypes.join(", ")}`
                          : ""
                      }.`
                    : "User key not found."}
                </p>
              ...
              filteredMediaData.length > 0 && (
                <div className="grid p-2 grid-cols-2 gap-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 sm:gap-3">
                  {filteredMediaData.map((media) => {
                    const isCurrentlySelectedInParentForm =
                      media.fileId !== null &&
                      currentSelectionIds.includes(media.fileId);
                    const isDialogSelected =
                      selectedMediaInDialog?.webMediaId === media.webMediaId;
                    const hasValidIdForSelection =
                      returnType === "id"
                        ? media.fileId !== null && media.fileId !== undefined
                        : !!media.fileUrl;
                    ...
                    return (
                      <div
                        ...
                        className={cn(
                          ...
                          isDialogSelected && hasValidIdForSelection
                            ? "ring-2 ring-primary ring-offset-1 shadow-lg border-primary"
                            : "border-border",
                          isCurrentlySelectedInParentForm &&
                            !isDialogSelected &&
                      ...
                      >
                        <div className="relative w-full aspect-[4/3] bg-muted flex-shrink-0">
                          ...
                          {isCurrentlySelectedInParentForm &&
                            ...
                            hasValidIdForSelection && (
                              <div
                                className="absolute top-1 right-1 p-0.5 bg-green-600 rounded-full shadow-md"
                                title="Already selected in form"
                              >
                        ...
                        <div className="p-1.5 sm:p-2 flex flex-col flex-grow justify-between">
                          <p
                            className="text-[10px] sm:text-xs text-foreground truncate font-medium leading-tight mb-1"
                            title={media.name}
                          >
                            {media.name}
                          </p>
                          <Button
                            ...
                            onClick={() => {
                              if (hasValidIdForSelection)
                                setSelectedMediaInDialog(media);
                            }}
        ...
        <DialogFooter className="p-3 border-t flex-shrink-0 flex flex-col sm:flex-row justify-between items-center gap-2">
          ...
          <div className="flex gap-2 order-2 sm:order-none">
            ...
            <Button
              ...
              onClick={() =>
                setCurrentPage((p) =>
                  Math.min(pagination?.pageCount || 1, p + 1)
                )
              }
              disabled={
                !pagination ||
