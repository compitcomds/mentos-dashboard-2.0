...
export default function UploadButton({
  ...
  const [fileToUpload, setFileToUpload] = React.useState<File | Blob | null>(
    null
  );
  ...
  const openEditor = () => {
    if (fileToUpload && fileToUpload.type.startsWith("image/") && previewUrl) {
      ...
      toast({
        variant: "destructive",
        title: "Cannot Edit",
        description: "Only image files can be edited.",
      });
    }
  };
  ...
  const handleUpload = async () => {
    ...
    const filename =
      fileToUpload instanceof File
        ? fileToUpload.name
        : `${mediaName || "edited-image"}.${
            fileToUpload.type.split("/")[1] || "png"
          }`;
    ...
    uploadMutation.mutate(
      {
        ...
        tags:
          selectedTags.length > 0
            ? selectedTags.map((tag) => ({ tag_value: tag }))
            : [],
      },
      {
        onSuccess: () => {
          ...
          toast({
            ...
            description: `Media "${
              mediaName || filename.split(".").slice(0, -1).join(".")
            }" uploaded.`,
          });
          onUploadSuccess?.();
          setTimeout(() => {
          ...
          {isEditorOpen && previewUrl ? (
            ...
            <>
              <DialogHeader>
                ...
                <DialogDescription>
                  Review the file details and add metadata. Max file size:{" "}
                  {MAX_FILE_SIZE_MB}MB.
                </DialogDescription>
              </DialogHeader>
              {!displayFile && (
                    ...
                    {selectedTags.length < 10 && PREDEFINED_TAGS.length > 0 && (
                      <div className="mt-2 flex flex-wrap gap-1.5">
                        {PREDEFINED_TAGS.filter(
                          (pt) => !selectedTags.includes(pt)
                        ).map((tag) => (
                          <Button
                            key={tag}
                            type="button"
                            variant="outline"
                            size="sm" // Custom smaller size for tag buttons
