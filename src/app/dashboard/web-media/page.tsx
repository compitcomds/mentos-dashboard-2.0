...
import { AlertCircle, Loader2, LayoutGrid, List, HardDrive, Search, X, Filter, ChevronLeft, ChevronRight } from "lucide-react";
...
import {
    ...
    TableBody,
    TableCell,
} from "@/components/ui/table"; // Added missing table imports
...
export default function WebMediaPage() {
    ...
    const [nameFilter, setNameFilter] = React.useState(''); // For filtering by WebMedia.name
    ...
    React.useEffect(() => { setCurrentPage(1); }, [categoryFilter, nameFilter]); // Reset page on filter change

    const handleUploadSuccess = () => {
        refetch();
        if (userResource?.documentId) { // Refetch storage after upload
            // This assumes userResource query key includes documentId or tenent_id for specific invalidation
            // Or just refetch the generic key if it's global for the user
            // queryClient.invalidateQueries({ queryKey: USER_RESOURCE_QUERY_KEY(userKey) });
        }
    };
...
function WebMediaPageSkeleton({ viewMode, pageSize }: { viewMode: ViewMode, pageSize: number }) {
    ...
    return (
      <div className="space-y-4">
        <Card><CardHeader className="pb-2"><Skeleton className="h-5 w-1/4" /></CardHeader><CardContent className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3 items-end pt-2">{[...Array(5)].map((_,i)=><Skeleton key={i} className="h-16 w-full"/>)}</CardContent></Card>
