...
export default function WebMediaPage() {
  ...
  const [viewMode, setViewMode] = React.useState<ViewMode>(() =>
    getStoredPreference("webMediaViewMode", "table")
  );
  const [currentPage, setCurrentPage] = React.useState(1);
  const [pageSize, setPageSize] = React.useState(() =>
    getStoredPreference(
      "webMediaPageSize",
      viewMode === "table" ? DEFAULT_PAGE_SIZE_TABLE : DEFAULT_PAGE_SIZE_CARD
    )
  );
  const [sortField, setSortField] = React.useState<SortField>(() =>
    getStoredPreference("webMediaSortField", "createdAt")
  );
  const [sortOrder, setSortOrder] = React.useState<SortOrder>(() =>
    getStoredPreference("webMediaSortOrder", "desc")
  );
  ...
  const [userDefinedTags, setUserDefinedTags] = React.useState<string[]>(() =>
    getStoredPreference(USER_DEFINED_TAGS_STORAGE_KEY, [])
  );
  const [selectedFilterTags, setSelectedFilterTags] = React.useState<string[]>(
    () => getStoredPreference(SELECTED_TAGS_STORAGE_KEY, [])
  );
  ...
  const {
    ...
    refetch,
    isFetching,
  } = useFetchMedia(mediaQueryOptions);
  const mediaItems = mediaDataResponse?.data || [];
  const pagination = mediaDataResponse?.meta?.pagination;

  const { data: userResource, isLoading: isLoadingUserResource } =
    useGetUserResource();

  React.useEffect(() => {
    setStoredPreference("webMediaViewMode", viewMode);
  }, [viewMode]);
  React.useEffect(() => {
    setStoredPreference("webMediaPageSize", pageSize);
    setCurrentPage(1);
  }, [pageSize]);
  React.useEffect(() => {
    setStoredPreference("webMediaSortField", sortField);
    setCurrentPage(1);
  }, [sortField]);
  React.useEffect(() => {
    setStoredPreference("webMediaSortOrder", sortOrder);
    setCurrentPage(1);
  }, [sortOrder]);
  React.useEffect(() => {
    setCurrentPage(1);
  }, [activeCategoryFilter, activeNameFilter, selectedFilterTags]);
  ...
  const storageProgress =
    totalStorageKB > 0 ? (usedStorageKB / totalStorageKB) * 100 : 0;
  ...
  return (
    <TooltipProvider>
      <div className="flex flex-col space-y-6">
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          ...
          <div className="flex items-center space-x-2">
            ...
            <UploadButton
              ...
              disabled={isLoadingUser || !userKey}
            />
          </div>
        </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-2 items-end">
                    <div className="relative md:col-span-1">
                      ...
                      <Input
                        ...
                        onKeyDown={(e) =>
                          e.key === "Enter" && applyTextFilters()
                        }
                        className="h-8 text-xs"
                        disabled={isLoadingMedia || isFetching}
                      />
                    </div>
                    <div className="relative md:col-span-1">
                      <Label
                        htmlFor="category-filter-input"
                        className="text-xs text-muted-foreground mb-1 block"
                      >
                        Filter by Category
                      </Label>
                    ...
                    <Button
                      ...
                      disabled={isLoadingMedia || isFetching}
                    >
                      <Search className="h-3.5 w-3.5 mr-1.5" /> Apply Text
                      Filters
                    </Button>
                  </div>
            ...
            {isLoadingUserResource && !userResource ? (
              ...
              <>
                ...
                <p className="text-sm text-muted-foreground">
                  Used {formatBytesForDisplay(usedStorageKB * 1024)} of{" "}
                  {formatBytesForDisplay(totalStorageKB * 1024)}
        ...
        {(isLoading && !mediaDataResponse) ||
        (isFetching && !mediaDataResponse) ? (
          <WebMediaPageSkeleton viewMode={viewMode} pageSize={pageSize} />
        ) : null}
        ...
        {isError && !isFetching && (
          <Alert variant="destructive">
            ...
            <AlertDescription>
              Could not fetch user or media files. Error:{" "}
