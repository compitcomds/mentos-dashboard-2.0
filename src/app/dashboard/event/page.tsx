...
const SORT_FIELD_OPTIONS_EVENT: { label: string; value: SortFieldEvent }[] = [
  ...
  { label: "Event Date", value: "event_date_time" },
  { label: "Category", value: "category" },
  { label: "Status", value: "event_status" },
...
export default function EventPage() {
  ...
  const viewMode =
    ...
    getStoredPreference("eventViewMode", "table");
    ...
    searchParams.get("limit") ||
      String(
        getStoredPreference(
          "eventPageSize",
          viewMode === "table"
            ? DEFAULT_PAGE_SIZE_EVENT_TABLE
            : DEFAULT_PAGE_SIZE_EVENT_CARD
        )
      ),
    10
  );
  const sortField =
    (searchParams.get("sortBy") as SortFieldEvent | null) ||
    getStoredPreference("eventSortField", DEFAULT_SORT_FIELD);
  const sortOrder =
    (searchParams.get("order") as SortOrderEvent | null) ||
    getStoredPreference("eventSortOrder", DEFAULT_SORT_ORDER);
  ...
  React.useEffect(() => {
    setLocalTitleFilter(activeTitleFilter || "");
  }, [activeTitleFilter]);
  ...
  const {
    ...
    refetch,
    isFetching,
  } = useGetEvents(eventQueryOptions);
  ...
  const handleViewModeChange = (newMode: ViewMode) => {
    ...
    setStoredPreference("eventViewMode", newMode);
  };
  const handlePageChange = (newPage: number) => updateUrl({ page: newPage });
  const handlePageSizeChange = (value: string) => {
    updateUrl({ limit: value, page: null });
    setStoredPreference("eventPageSize", Number(value));
  };
  const handleSortFieldChange = (value: SortFieldEvent) => {
    updateUrl({ sortBy: value, page: null });
    setStoredPreference("eventSortField", value);
  };
  const handleSortOrderChange = (value: SortOrderEvent) => {
    updateUrl({ order: value, page: null });
    setStoredPreference("eventSortOrder", value);
  };
  const handleCategoryFilterChange = (value: string | null) =>
    updateUrl({ category: value, page: null });
  const handleStatusFilterChange = (value: string | null) =>
    updateUrl({ status: value, page: null });
  const applyTitleFilter = () =>
    updateUrl({ search: localTitleFilter.trim() || null, page: null });
  ...
  const handleDelete = (eventToDelete: Event) => {
    if (!eventToDelete.documentId) {
      toast({
        variant: "destructive",
        title: "Error",
        description: "Cannot delete event: missing identifier.",
      });
      return;
    }
    deleteMutation.mutate(
      {
        ...
        numericId: String(eventToDelete.id),
      },
      {
        onSuccess: () => {
      ...
      <div className="flex flex-col space-y-6">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          ...
          <div className="flex items-center space-x-2 self-end sm:self-center">
            ...
            <Link
              href={`/dashboard/event/new?returnUrl=${encodeURIComponent(
                currentFullUrl
              )}`}
            >
              ...
              <AccordionItem value="item-1">
                ...
                <AccordionContent className="pt-3 space-y-3">
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-2 items-end">
                    <div>
                      ...
                      <Input
                        ...
                        onKeyDown={(e) =>
                          e.key === "Enter" && applyTitleFilter()
                        }
                        className="h-8 text-xs"
                        disabled={isLoadingEvents || isFetching}
                      />
                    </div>
                    <div>
                      <Label
                      ...
                      >
                        ...
                        <SelectContent>
                          {["all", ...EVENT_CATEGORIES_MOCK].map((cat) => (
                            <SelectItem
                              key={cat}
                              value={cat}
                  ...
                  <Button
                    ...
                    size="sm"
                    className="h-8 text-xs mt-2 px-3 py-1"
                    disabled={isLoadingEvents || isFetching}
                  >
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 items-end pt-3 border-t mt-3">
                    <div>
                      ...
                      <Select
                        value={sortField}
                        onValueChange={(value) =>
                          handleSortFieldChange(value as SortFieldEvent)
                        }
                        disabled={isLoadingEvents || isFetching}
                      >
                        ...
                        <SelectContent>
                          {SORT_FIELD_OPTIONS_EVENT.map((opt) => (
                            <SelectItem
                              key={opt.value}
                              value={opt.value}
                              className="text-xs"
                    ...
                    <div>
                      ...
                      <Select
                        value={sortOrder}
                        onValueChange={(value) =>
                          handleSortOrderChange(value as SortOrderEvent)
                        }
                        disabled={isLoadingEvents || isFetching}
                      >
                        <SelectTrigger className="h-8 text-xs">
                        ...
                        <SelectContent>
                          {SORT_ORDER_OPTIONS_EVENT.map((opt) => (
                            <SelectItem
                              key={opt.value}
                              value={opt.value}
                              className="text-xs"
                    ...
                    <div>
                      ...
                      >
                        ...
                        <SelectContent>
                          {PAGE_SIZE_OPTIONS_EVENT.map((opt) => (
                            <SelectItem
                              key={opt.value}
                              value={opt.value}
                              className="text-xs"
        ...
        {(isLoading && !eventDataResponse) ||
        (isFetching && !eventDataResponse) ? (
          <EventPageSkeleton viewMode={viewMode} pageSize={pageSize} />
        ) : null}
        ...
        {!isLoading && !isError && userKey && events.length === 0 && (
          <div className="mt-4 border border-dashed border-border rounded-md p-8 text-center text-muted-foreground">
            No events found for your key ({userKey}) matching the current
            filters. Click "New Event" to create one.
          </div>
        )}
            ...
            <Card>
              ...
              <CardContent>
                <Table>
                  ...
                  <TableBody>
                    {events.map((event) => {
                      const eventDateTime = event.event_date_time
                        ? parseISO(String(event.event_date_time))
                        : null;
                      const editLink = `/dashboard/event/${
                        event.documentId || event.id
