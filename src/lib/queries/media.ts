...
const MEDIA_QUERY_KEY = (
  userKey?: string,
  options?: Omit<FetchMediaFilesParams, "userTenentId">
) => [
  "mediaFiles",
  userKey || "all",
  options?.page,
...
export function useFetchMedia(
  ...
  return useQuery<FindMany<CombinedMediaData>, Error>({
    queryKey: MEDIA_QUERY_KEY(userTenentId, {
      page,
      pageSize,
      sortField,
      sortOrder,
    ...
    queryFn: () => {
      if (!userTenentId) {
        console.warn(
          "useFetchMedia: User key not provided. Returning empty result."
        );
...
export function useUploadMediaMutation() {
  ...
  >({
    mutationFn: async ({ file, name, alt, category, tags }) => {
      ...
      console.log(
        `[useUploadMediaMutation] Response from uploadFile service:`,
        uploadResponseArray
      );

      if (
        !uploadResponseArray ||
        uploadResponseArray.length === 0 ||
        typeof uploadResponseArray[0].id !== "number"
      ) {
        console.error(
          "[useUploadMediaMutation] File upload response did not contain expected data (numeric ID) or failed. Response:",
          uploadResponseArray
        );
        throw new Error(
          "File upload response did not contain expected data (numeric ID) or failed."
        );
      }

      const uploadedFile = uploadResponseArray[0];
      console.log(
        `[useUploadMediaMutation] Uploaded file details from service:`,
        uploadedFile
      );

      if (!uploadedFile.id) {
        console.log(
          "[useUploadMediaMutation] Uploaded file does not have a valid ID. Cannot proceed with creating WebMedia."
        );
        throw new Error(
          "Uploaded file does not have a valid ID. Cannot proceed with creating WebMedia."
        );
      }
      ...
      console.log(
        `[useUploadMediaMutation] Payload for createWebMedia service:`,
        createPayload
      );

      const webMediaResponse = await createWebMedia(createPayload);
      console.log(
        `[useUploadMediaMutation] Response from createWebMedia service:`,
        webMediaResponse
      );
    ...
    onSuccess: (data) => {
      console.log(
        `[useUploadMediaMutation] onSuccess: Invalidating media queries for userKey: ${userKey}`
      );
    ...
    onError: (error: unknown) => {
      ...
      if (error instanceof AxiosError) {
        ...
        console.error(
          "[useUploadMediaMutation] Upload/Create WebMedia Axios Error:",
          error.response?.data || error
        );
      } else if (error instanceof Error) {
        message = error.message;
        console.error(
          "[useUploadMediaMutation] Upload/Create WebMedia Error:",
          error
        );
      } else {
        console.error(
          "[useUploadMediaMutation] Unknown Upload/Create WebMedia Error:",
          error
        );
      }
      toast({
...
export function useUpdateMediaMutation() {
  ...
  return useMutation<
    WebMedia,
    Error,
    { webMediaId: number; payload: UpdateWebMediaPayload }
  >({
    mutationFn: ({ webMediaId, payload }) =>
      updateWebMedia(webMediaId, payload),
    onSuccess: (data) => {
      toast({
        title: "Success!",
...
export function useDeleteMediaMutation() {
  ...
  return useMutation<
    { webMediaDeleted: boolean; fileDeleted: boolean },
    Error,
    { webMediaId: number; fileId: number | null }
  >({
    mutationFn: ({ webMediaId, fileId }) =>
      deleteMediaAndFile(webMediaId, fileId),
    onSuccess: (data, variables) => {
      ...
      if (variables.fileId !== null) {
        description += data.fileDeleted
          ? ` Associated file (ID: ${variables.fileId}) also deleted.`
          : ` Could not delete associated file (ID: ${variables.fileId}). Check server logs and permissions.`;
      } else {
      ...
      toast({
        ...
        variant:
          variables.fileId !== null && !data.fileDeleted
            ? "destructive"
            : "default",
      });
      queryClient.invalidateQueries({ queryKey: MEDIA_QUERY_KEY(userKey) });
  ...
  return useQuery<Media | null, Error>({
    ...
    queryFn: () => {
      if (id === null || id === undefined) {
        console.warn(
          "useGetMediaFileDetailsById: id is null or undefined. Returning null."
        );
