...
import {
  getQueryForms as getQueryFormsService,
  getQueryForm as getQueryFormService,
  type GetQueryFormsParams,
} from "@/lib/services/query-form";
...
const QUERY_FORMS_QUERY_KEY = (
  userTenentId?: string,
  options?: UseGetQueryFormsOptions
) => [
  "queryForms",
  userTenentId || "all",
  options?.type || "allTypes",
  options?.group_id || "allGroups",
  options?.page || 1,
  options?.pageSize || 10,
  options?.sortField,
  options?.sortOrder,
  ...
  options?.fetchAllForExport,
];

const QUERY_FORM_DETAIL_QUERY_KEY = (id?: string, userTenentId?: string) => [
  "queryForm",
  id || "detail",
  userTenentId || "all",
];
...
export const useGetQueryForms = (options?: UseGetQueryFormsOptions) => {
  ...
  return useQuery<FindMany<QueryForm>, Error>({
    ...
    queryFn: () => {
      if (!userTenentId) {
        console.warn(
          "useGetQueryForms: User tenent_id not available. Returning empty result."
        );
      ...
      const params: GetQueryFormsParams = {
        userTenentId,
        type,
        group_id,
        page: effectivePage,
